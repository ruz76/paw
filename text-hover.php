...
final class c2c_TextHover extends c2c_TextHover_Plugin_044 {
	...
	protected function load_config() {
		...
		$this->config = array(
			'text_to_hover' => array(
				...
				'datatype'         => 'hash',
				'default'          => array(
					"WP" => "WordPress", "OS" => "Ostrava", "OP" => "Opava", "P1" => "Prdel1", "P2" => "Prdel2"
				),
				'allow_html'       => true,
				'no_wrap'          => true,
	...
	public function enqueue_scripts() {
		...
		/*Added for Map*/
		wp_enqueue_style( 'leaflet', 'https://unpkg.com/leaflet@1.0.1/dist/leaflet.css');
		wp_enqueue_script( 'jquery-1.12.4', 'https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js');
		wp_enqueue_script( 'leaflet-1.0.1', 'https://unpkg.com/leaflet@1.0.1/dist/leaflet.js');
		wp_enqueue_script( 'showplace', 'http://gisak.vsb.cz/ruzicka/lib/leaflet/showplace.js');
		wp_enqueue_script( 'map', plugins_url( 'assets/map.js', __FILE__ ));
		/*End Added for Map*/
	}
	 ...
	 * Added for Map
	 *
	 * @param string  $text Text to be searched in places table
	 * @return string lat lon fragment for element acronym.
	 */
	public function getCoords( $text ) {

		/*
		Simple Testing database:
		create table places (place varchar(255), lat float, lon float);
		insert into places values ('Ostrava', 49.9, 18.5);
		insert into places values ('Prdel1', 48.9, 18.5);
		insert into places values ('Prdel2', 48.9, 17.5);
		*/
		...
		*/
		...
		$mysqli = new mysqli('localhost', '*', '*', 'wordpress');
		...
		$sql = "SELECT lon, lat FROM places WHERE place LIKE '%".$text."%'";
		//echo $sql;
		...
		return "lat='".$actor['lat']."' lon='".$actor['lon']."'";
	}
	...
	public function text_hover( $text ) {
		...
		foreach ( (array) $text_to_hover as $old_text => $hover_text ) {
			...
			/*Added for Map*/
			$coords = $this->getCoords($hover_text);
			/*Changed for map*/
			$new_text = "<acronym ".$coords." zoom='12' text='".$hover_text."' class='c2c-text-hover' title='" . esc_attr( addcslashes( $hover_text, '\\$' ) ) . "'>\\1</acronym>";
			/*End of Changed for map*/
